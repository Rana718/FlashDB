...
- **`FYRODB_MAX_KEYS`** — admission limit for live keys. Shards start with eight slots and grow lazily, so a high limit no longer reserves the full table at startup.
